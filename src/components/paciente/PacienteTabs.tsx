...
const formatarData = (val: unknown): string => {
  const d = toDate(val);
  if (!d) return "-";
...
export default function PacienteTabs({ patient, children }: PacienteTabsProps) {
  ...
  const idade =
    patient?.idade ??
    calcularIdade(patient?.["data de nascimento"]) ??
    "-";
    ...
    <div className="w-full">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-4">
        ...
        <div className="flex flex-wrap gap-3 text-sm text-slate-500 mt-1">
          ...
          {dataNascFmt !== "-" && (
            <span>• Nascimento: {dataNascFmt}</span>
          )}
      ...
      <div className="min-h-[200px]">
        {abaAtiva === "dados" &&
          (children?.dados ?? <DadosPaciente patient={patient} />)}
...
function DadosPaciente({ patient }: { patient: Patient }) {
  const campos = [
    { label: "Nome completo", valor: patient?.["nome completo"] },
    {
      ...
      valor: formatarData(patient?.["data de nascimento"]),
    },
    {
      label: "Idade",
      valor: patient?.idade
        ? `${patient.idade} anos`
        : `${calcularIdade(patient?.["data de nascimento"]) ?? "-"} anos`,
    },
    { label: "CPF", valor: patient?.cpf ?? "-" },
    { label: "Peso", valor: patient?.peso ? `${patient.peso} kg` : "-" },
    {
      label: "Endereço",
      valor: patient?.endereco
