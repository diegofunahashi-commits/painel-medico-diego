...
import { FileText, Clock, Repeat, Trash2, Printer, ChevronRight, Download } from 'lucide-react';
        ...
        <div className="space-y-3">
          {receitas.map((receita) => (
            <div key={receita.prescriptionId}>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-slate-900">Receita {receita.prescriptionId}</p>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {receita.medicacoes?.map((med, idx) => (
                        <span key={idx} className="px-2 py-1 bg-slate-100 text-slate-700 text-xs rounded-md font-medium">
                          {med.nome}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); handleGerarPDF(receita); }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm text-slate-600">
                          ...
                          <p><span className="font-medium">Peso:</span> {receita.peso ? `${receita.peso} kg` : '-'}</p>
                          <p><span className="font-medium">CPF:</span> {receita.cpf || '-'}</p>
                          <p>
                            <span className="font-medium">Endereço:</span>{' '}
                            {receita.endereco
                              ? `${receita.endereco.rua || ''}, ${receita.endereco.numero || ''} — ${receita.endereco.cidade || ''}`
                              : '-'}
                          </p>
                        </div>
                      </div>
              ...
              <div
                ref={(el) => { pdfRefs.current[receita.prescriptionId] = el; }}
              ...
              >
                <div style={{ width: '210mm', minHeight: '297mm', padding: '20mm', fontFamily: 'Arial, sans-serif', background: '#fff', color: '#000' }}>
                  ...
                  <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                    <h2 style={{ fontSize: '18px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '2px', margin: 0 }}>Receituário Médico</h2>
                  ...
                  <div style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px', marginBottom: '24px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px' }}>
                      <p><strong>Nome:</strong> {receita.patientId || 'Paciente'}</p>
                      <p><strong>CPF:</strong> {receita.cpf || '-'}</p>
                      <p><strong>Idade:</strong> {receita.idade || '-'} anos</p>
                      <p><strong>Peso:</strong> {receita.peso ? `${receita.peso} kg` : '-'}</p>
                      <p style={{ gridColumn: '1 / -1' }}>
                        <strong>Endereço:</strong>{' '}
                        {receita.endereco
                          ? `${receita.endereco.rua || ''}, ${receita.endereco.numero || ''}, ${receita.endereco.bairro || ''}, ${receita.endereco.cidade || ''}`
                          : '-'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: '32px' }}>
                    <h3 style={{ fontSize: '13px', fontWeight: 'bold', color: '#0c4a6e', marginBottom: '16px', textTransform: 'uppercase' }}>Prescrição</h3>
                    {receita.medicacoes?.map((med, idx) => (
                      <div key={idx} style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px', marginBottom: '12px' }}>
                        <p style={{ fontSize: '15px', fontWeight: 'bold', margin: '0 0 8px 0' }}>{idx + 1}. {med.nome}</p>
                        <p style={{ fontSize: '13px', margin: '4px 0', color: '#334155' }}><strong>Dose:</strong> {med.dose}</p>
                        <p style={{ fontSize: '13px', margin: '4px 0', color: '#334155' }}><strong>Via:</strong> {med.via}</p>
                        <p style={{ fontSize: '13px', margin: '4px 0', color: '#334155' }}><strong>Frequência:</strong> {med.frequencia}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ position: 'absolute', bottom: '20mm', left: '20mm', right: '20mm', textAlign: 'center', fontSize: '10px', color: '#94a3b8', borderTop: '1px solid #e2e8f0', paddingTop: '8px' }}>
                    <p>Documento gerado eletronicamente — {formatarData(receita.createdAt, "dd/MM/yyyy 'às' HH:mm")}</p>
